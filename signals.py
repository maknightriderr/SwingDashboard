"""
signals.py v11 — Production-Grade Signal Engine
Fixes: Unified ATR Risk Engine, 20-day S/R, EMA labels corrected,
       trail_stop RR explosion, News parser for yfinance v1.4.x + RSS fallback.
"""
...
def detect_candlesticks(open_p, high, low, close):
    ...
    if not c0 or not c1: return candles
...
# FIX: S/R windows upgraded to 20-day (was 7-day) for proper swing trade context.
# FIX: EMA labels corrected — ema9/ema21/ema50 with clear naming.
def _compute_indicators_raw(symbol, period="1y", prefetched_df=None):
    ...
    cmp = float(close.iloc[-1])
    if pd.isna(cmp) or cmp <= 0: return None

    rsi_series = compute_rsi_wilder(close, 14)
    rsi = round(float(rsi_series.iloc[-1]), 1) if not pd.isna(rsi_series.iloc[-1]) else None

    # ── EMAs — correctly named for swing trading ──────────────────────────────
    ema9  = float(close.ewm(span=9).mean().iloc[-1])    # fast momentum EMA
    ema21 = float(close.ewm(span=21).mean().iloc[-1])   # swing EMA (replaces ema50 label)
    ema50 = float(close.ewm(span=50).mean().iloc[-1])   # trend EMA (replaces ema200 label)
    ...
    # ── FIX: S/R upgraded from 7-day to 20-day for swing context ─────────────
    ...
    # Trend uses correctly named EMAs
    trend = "Sideways"
    ...
    try:
        ...
        for i in range(len(close)):
            if i < 7 or pd.isna(upper.iloc[i]):
                st.iloc[i] = float(close.iloc[i]); dir_.iloc[i] = 1; continue
            prev_st   = float(st.iloc[i - 1])
            prev_dir  = dir_.iloc[i - 1]
            curr_close = float(close.iloc[i])
            if prev_dir == 1:
                if curr_close < prev_st: dir_.iloc[i] = -1; st.iloc[i] = float(upper.iloc[i])
                else: dir_.iloc[i] = 1; st.iloc[i] = max(float(lower.iloc[i]), prev_st)
            else:
                if curr_close > prev_st: dir_.iloc[i] = 1; st.iloc[i] = float(lower.iloc[i])
                else: dir_.iloc[i] = -1; st.iloc[i] = min(float(upper.iloc[i]), prev_st)
    ...
    return {
        ...
        # Correctly labelled EMAs
        "ema9": round(ema9, 2), "ema21": round(ema21, 2), "ema50": round(ema50, 2),
        # Legacy aliases so app.py display code still works without changes
...
# ==============================================================================
# ─── UNIFIED RISK ENGINE ──────────────────────────────────────────────────────
# This is the single source of truth for all target/SL/RR calculations.
# Both generate_signals() and find_sector_picks() now call this.
# FII logic: tight ATR-based entries, realistic 2-2.5x RR, 2*ATR stop.
# ==============================================================================
def _calc_risk_params(cmp, atr, resistance, buy_at=None, pct=None,
                      supertrend_val=None, supertrend_bullish=None,
                      action="HOLD"):
    """
    Unified risk-sizing engine.

    For HOLD/AVERAGE on existing positions:
      - Target  = max(20-day resistance, cmp + 2.5*ATR)  — realistic swing target
      - SL      = cmp - 2.0*ATR                          — ATR-based, not trail_stop
      - Trail   = supertrend if bullish, else cmp-2*ATR

    For new SECTOR PICKS:
      - Target  = max(cmp * 1.10, cmp + 2.5*ATR)
      - SL      = cmp - 1.25*ATR                         — tighter for fresh entries

    RR is always computed on these consistent numbers, so 16x phantom RR is impossible.
    """
    if action in ("HOLD", "AVERAGE", "WATCH"):
        # Existing position: use 2*ATR stop (wider = more room to breathe)
        stop_loss = round(cmp - 2.0 * atr, 2)
        # If supertrend is bullish and above our ATR stop, use it as trailing floor
        ...
        # Fresh entry (SELL exit or new pick)
        stop_loss = round(cmp - 2.0 * atr, 2)
        target    = round(cmp, 2)
...
def generate_signals(trades_df):
    ...
    for _, row in open_trades.iterrows():
        ...
        # FIX: use correctly named EMAs
        ema9, ema21, ema50 = ind["ema9"], ind["ema21"], ind["ema50"]
        # 20-day S/R (fixed from 7-day)
        ...
        # FIX: Use ATR-based trail stop, not max(buy_at, st_val) which causes RR explosion
        atr_trail = round(cmp - 2.0 * atr, 2)
        if st_bullish and st_val:
            effective_trail = round(max(atr_trail, st_val), 2)
        else:
            effective_trail = atr_trail
        ...
        if rsi and 45 <= rsi <= 65 and cmp > ema9 and stock_rs_strong:
            hold.append(f"RSI neutral, Above EMA9, Beating Index")
        ...
        # ── Determine action & compute risk using UNIFIED engine ──────────────
        if sell:
            action       = "🔴 SELL"
            reason_base  = " | ".join(sell)
            strength     = min(len(sell) * 25 + 15, 95)
            # For SELL: target is CMP (exit now), SL is re-entry level
            target, stop_loss, rr = cmp, round(cmp - 2.0 * atr, 2), None
            ...
            # Use unified engine for AVERAGE
            ...
            reason_base  = hold[0]
            strength     = 55
            # FIX: Use unified engine — eliminates phantom RR=16 bug
            target, stop_loss, rr = _calc_risk_params(
                cmp, atr, resistance, buy_at=buy_at, pct=pct,
        ...
        signals.append({
            ...
            "cmp": cmp, "rsi": rsi,
            "ema20": ema9, "ema50": ema21,   # legacy labels for app.py display
            "atr": atr,
...
def sector_rotation(trades_df=None):
    ...
    nifty_ret1m = 0.0; nifty_ret3m = 0.0
    ...
    for sector, idx_sym in SECTOR_INDICES.items():
        try:
            ...
            if df_idx is None or len(df_idx) < 21: continue
            close   = df_idx["Close"]
            ...
            elif rs_ratio > 100 and rs_momentum <= 100: rrg_quadrant = "📉 Weakening"
            elif rs_ratio <= 100 and rs_momentum > 100: rrg_quadrant = "🔄 Improving"
            ...
            momentum_score = round(rsi_score * 0.20 + rs_score * 0.40 + trend_score * 0.20 + macd_score * 0.20, 3)
            rows.append({
                "sector": sector,
                ...
                "rrg_quadrant": rrg_quadrant, "trend_days": trend_duration_days,
...
# ─── Sector Stock Discovery (uses unified risk engine) ────────────────────────
def find_sector_picks(selected_sectors=None, max_per_sector=3):
    ...
    for sector in sectors:
        ...
        for symbol in SECTOR_STOCKS.get(sector, []):
            ...
            # Unified engine for picks: 1.25*ATR SL (tighter fresh entry)
            atr = ind["atr"]
            ...
            all_pats = patterns + candles; pat_str = " | ".join(all_pats) if all_pats else ""
...
def generate_market_scanner():
    ...
    for symbol in all_symbols:
        ...
        if "📈 Double Top" in patterns or "🏔️ Head & Shoulders (Top)" in patterns or "🟥 Bearish Engulfing" in candles: score -= 5
        ...
        all_pats = patterns + candles; pat_str = " | ".join(all_pats) if all_pats else "—"
        atr = ind["atr"]; sup, res = ind["support"], ind["resistance"]
        sl  = round(cmp - 1.25 * atr, 2)
        tgt = round(max(cmp * 1.10, cmp + 2.5 * atr), 2)
        results.append({
            "Generated": datetime.now().strftime("%d %b %H:%M"), "Sector": sector,
            "Stock": symbol, "CMP": float(cmp), "Entry": float(cmp),
            "Target": float(tgt), "SL": float(sl), "Support": float(sup),
            "Resist": float(res), "Signal": signal, "Score": score,
...
# ==============================================================================
# ─── NEWS ENGINE (FIXED) ──────────────────────────────────────────────────────
# Fix 1: Handle yfinance v1.4.x news structure (content dict, not flat title/link)
# Fix 2: Add Google News RSS fallback — works without Yahoo auth, server-safe
# Fix 3: Proper HTML sanitization for Streamlit display
# ==============================================================================

def _parse_yf_news_item(item):
    """
    Parse a single yfinance news item.
    yfinance v1.4.x: item = { 'content': { 'title': '...', 'clickThroughUrl': {'url': '...'} } }
    yfinance legacy:  item = { 'title': '...', 'link': '...' }
    """
    if not isinstance(item, dict):
        return None, None

    # New yfinance v1.4.x structure
    content = item.get("content", {})
    if isinstance(content, dict) and content.get("title"):
        title = content.get("title", "")
        # clickThroughUrl or canonicalUrl
    ...
    # Legacy structure (yfinance < 0.2.x)
...
def _fetch_google_news_rss(query, max_items=2):
    """
    Fallback: fetch headlines via Google News RSS.
    No auth required, works reliably on servers.
    Returns list of (title, link) tuples.
    """
    try:
...
def fetch_portfolio_news(open_trades_df):
    """
    Fetches latest news headlines for active holdings.
    Strategy:
      1. Try yfinance (handles both v1.4.x and legacy structure)
      2. Fallback to Google News RSS per symbol
    Returns list of HTML-formatted strings for Streamlit display.
    """
    ...
    for sym in unique_symbols:
        clean_sym = sanitize_ticker(sym)   # reuse existing sanitizer — no .NS.NS
        found = False

        # ── Try yfinance (primary) ────────────────────────────────────────────
        try:
            t     = yf.Ticker(f"{clean_sym}.NS")
            items = t.get_news(count=3)   # explicit count, not default property
            if items:
                for item in items[:2]:
